...
---
# Lab 2 - Working with vector data {#lab2}

The purpose of these exercises is for you to learn and understand the main GIS operation involving vector data. It will cover both geometry operations and attribute operations. Try to apply all the steps you have already learned on previous modules here as well: create a project folder, organize your data, save a named project with proper projection info, etc. The expectation is that you build up upon previous activities on each consecutive lab, so towards the end of the module you feel empowered to carry on with your own project.


Reference readings and video for this lab are:



## Guided Exercise 1 - Basic work with Vector Data

1. Dowload the data for this exercise from this link: . Extract the zipfile and organize your project folder and QGIS project file before you start.

  - Load all datasets in QGIS and inspect them:
...
3. Rename your layers on the layer list using human-readable, informative names. Save your project

4. Organize the *layer order* and play with different layer superpositions to make the best visualization for all datasets involved. Then experiment with the *symbology* of each layer to improve your visualization.
...
4. Turn on the Rivers layer, and open its attribute table. Click on the `Select features using expression` button: ![https://github.com/qgis/QGIS/blob/master/images/themes/default/mIconExpressionSelect.svg]  
...
6. Change your expression to `"SYSTEM" = 'Amazon'`. On the bottom left of the Attibute Table, select `Show Selected Features`. How many rivers are there in the Amazon system, according to this dataset?
...
8. Go back to the *Attibute Table* window for the Rivers layer. Click on the `Field Calculator` tool. On the new window, **uncheck** the `Only update selected features` if it is checked. The name the new field `Miles_Km`, change the output type to  `Decimal Number`. On the expression window, write `"MILES"  /  "KILOMETERS"`. Note the warning at the bottom. Once you click `OK`, turn off editing mode by clicking on the icon. Save you layer when asked to.

```{block type='rmdnote'}
**Stop and think:** Does the Miles to Km conversion factor you calculated seem right?
```

9. Click on the `Delete Field` button, and select `Miles_Km`. Click `OK`. Then click on the `Deselect All` button, to clear your previosu selection of Amazon system rivers.

10. Click on the `Add Field` button. Create a field named `Region`, of `Text(string)` type, and `Length` of `50`. Click `OK`. Return to the `Field Calculator`, and select `Update Existing Field` instead. Select `Region` as the field to be updated, and then simply type `'Unknown'` on the `Expression` window. Don't forget the quotes. Click `OK`. Exit edit mode choosing yes to save your edits. Save your project!


## Guided Exercise 3 -  Spatial Queries

1. Go to `Vector > Research Tools > Select by Location...`. Select the Earthquakes layer fort `Select features from`. Then select only the `Are within` option. As the comparison layer, choose the Countries Layer. Take a look at the options under `Modify current selection by`, and then set it at `Creating New Selection`. Click `Run`, and after it is finished, `Close`.
...
4. On the Attribute Table window, click on 'Invert Selection'. Then return to the `Field Calculator` and *Update* the `Origin` field by designating the newly selected features as `'Ocean'`.

5. Style your points by 'Origin' using the `Symbology` options.

6. On the World layer, use Attribute selection to select all countries from South America. Then go to `Vector > Research Tools > Select by Location...` and select all Earthquakes that are contained within the Countries layer, but this time check the option `Select features only`. 

7. Right-click on the Earthquakes layer name on the left panel and choose `Export > Save selected features as...`. Name you exported file as `Land_earthquakes_2011_south_america`, choose the appropriate folder to save it within your project structure by clickin on the `...` button to the left of the file name box, and select your format of choice (Geopackage or Shapefile). Click on `OK`. Save your project.
